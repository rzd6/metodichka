...
export function ExamsSection() {
  ...
  const copyToClipboard = (text: string, id: string, examId?: string, index?: number) => {
    navigator.clipboard.writeText(text)
    setCopiedIndex(id)
    if (examId !== undefined && index !== undefined) {
      setNextHighlight((prev) => ({ ...prev, [examId]: index + 1 }))
    }
  ...
  const renderContent = (content: string[], examId: string, answers?: string[]) => {
    ...
    return content.map((line, index) => {
      ...
      const isNext = nextHighlight[examId] === index

...
export function EventsSection() {
  ...
  const copyToClipboard = (text: string, id: string, eventId: string, index: number) => {
    navigator.clipboard.writeText(text)
    setCopiedIndex(id)
    setNextHighlight((prev) => ({ ...prev, [eventId]: index + 1 }))
  ...
  const renderContent = (content: string[] | undefined, eventId: string) => {
    ...
    return content.map((line, index) => {
      ...
      const isNext = nextHighlight[eventId] === index
    ...
    <div className="space-y-6 opacity-95">
      <div className="flex items-center gap-3 pb-4 border-b" style={{ borderColor: getTieColor() + "40" }}>
        ...
        <div className="ml-auto">
          <BugReportButton sectionName="Мероприятия" />
        </div>
      </div>

...
export function GovWaveSection() {
  ...
  const copyToClipboard = (text: string, id: string, categoryId: string, index: number) => {
    navigator.clipboard.writeText(text)
    setCopiedIndex(id)
    setNextHighlight((prev) => ({ ...prev, [categoryId]: index + 1 }))
          ...
          >
            ...
            <AccordionContent className="px-6 pb-6 space-y-3">
              {category.data.map((text, index) => {
                ...
                const isNext = nextHighlight[category.id] === index
                return (
                  <button
                    key={index}
                    onClick={() => copyToClipboard(text, lineId, category.id, index)}
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <p className={`text-sm flex-1 ${isNext ? "text-green-400 font-medium" : theme.mode === "dark" ? "text-white/90" : "text-gray-900"}`}>
                        {text}
                      </p>
                      <div

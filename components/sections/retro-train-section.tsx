...
export function RetroTrainSection() {
  ...
  const copyToClipboard = (text: string, id: string, retroId: string, index: number) => {
    navigator.clipboard.writeText(text)
    setCopiedIndex(id)
    setNextHighlight((prev) => ({ ...prev, [retroId]: index + 1 }))
  ...
  const renderContent = (content: string[], retroId: string) => {
    return content.map((line, index) => {
      ...
      const isNext = nextHighlight[retroId] === index

      return (
        <button
          key={index}
          onClick={() => copyToClipboard(line, lineId, retroId, index)}
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <p className={`text-sm flex-1 ${isNext ? "text-green-400 font-medium" : theme.mode === "dark" ? "text-white/90" : "text-gray-900"}`}>{line}</p>
            <div

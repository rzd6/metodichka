...
export function InterviewsSection() {
  ...
  const copyToClipboard = (text: string, id: string, interviewId: string, index: number) => {
    navigator.clipboard.writeText(text)
    setCopiedIndex(id)
    setNextHighlight((prev) => ({ ...prev, [interviewId]: index + 1 }))
  ...
  const renderContent = (content: string[], interviewId: string) => {
    return content.map((line, index) => {
      ...
      const isNext = nextHighlight[interviewId] === index
